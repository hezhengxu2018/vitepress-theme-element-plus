...
const props = defineProps<{
  title: string
  sites: Array<{ name: string, desc: string, avatar: string, url: string, type: string }>
}>();

const lessSites = props.sites.slice(0, 20).filter(item => !item.type);
const list = [...lessSites, ...lessSites];
</script>

<template>
  <div class="demo-template-cards">
    <div v-if="title" class="demo-template-title">
      {{ title }}
    </div>
    <div style="width: fit-content">
      <ul class="demo-template reverse">
        <li v-for="item in list" :key="item.url" class="demo-template-column">
          <a
            class="demo-template-card"
            :href="item.url"
            target="_blank"
            rel="noopener noreferrer"
          >
            <div class="demo-template-avatar">
              <img alt="" style="top: -2px; position: relative" :src="item.avatar">
            </div>
            <div>
